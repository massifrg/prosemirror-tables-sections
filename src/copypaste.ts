...
import { isTableSection, isTableSectionRole, tableNodeTypes } from './schema';
...
function growTable(
  ...
  let emptyHead;
  if (width > map.width) {
    const lastCellOfRow: Node[] = [];
    for (let row = 0; row < map.height; row++) {
      const lastCell = table.nodeAt(map.map[(row + 1) * map.width - 1])!;
      // console.log(`row ${row}, last child type: ${lastCell.type.name}`);
    ...
    for (let row = 0; row < map.height; row++) {
      const { node: rowNode, pos: rowPos } = getRow(table, row);
      const rowEnd = rowPos + rowNode!.nodeSize - 1;
      const cells: Node[] = [];
