...
export class CellSelection extends Selection {
  ...
  // bottom of the table.
  public isColSelection(tableMap?: TableMap): boolean {
    const table = this.$anchorCell.node(-2);
    const tableStart = this.$anchorCell.start(-2);
    ...
    const anchorBottom: number =
      anchorTop + this.$anchorCell.nodeAfter!.attrs.rowspan;
    const headBottom: number =
      headTop + this.$headCell.nodeAfter!.attrs.rowspan;
  ...
  // and head cell.
  public static sectionSelection(
    $anchorCell: ResolvedPos,
    $headCell: ResolvedPos = $anchorCell,
  ): CellSelection {
    const table = $anchorCell.node(-2);
