...
describe('pastedCells', () => {
  ...
  it('returns simple cells', () =>
    test(doc(table(tbody(tr('<a>', cEmpty, cEmpty, '<b>')))), 2, 1, [
      [cEmpty, cEmpty],
    ]));
  ...
  it('returns cells when the cursor is inside them', () =>
    test(table(tbody(tr(td(p('<a>foo')), td(p('<b>bar'))))), 2, 1, [
      [td(p('foo')), cEmpty],
    ]));

  it('returns multiple rows', () =>
    test(
      table(tbody(tr('<a>', cEmpty, cEmpty), tr(cEmpty, c11), '<b>')),
      2,
      2,
      [
        [cEmpty, cEmpty],
  ...
  it('will enter a fully selected table', () =>
    test(doc('<a>', table(tbody(tr(c11))), '<b>'), 1, 1, [[c11]]));

  it('can normalize a partially-selected row', () =>
    test(
      table(tbody(tr(td(p(), '<a>'), cEmpty, c11), tr(c11, c11), '<b>')),
      2,
      2,
      [
        [cEmpty, c11],
...
describe('insertCells', () => {
  function test(
    ...
    if (log) {
      logNode(state.doc, 'RESULT:', false);
      console.log('');
      logNode(expected_result, 'EXPECTED RESULT:', false);
    }
  ...
  it('keeps the original cells', () =>
    test(
      doc(table(tbody(tr(cAnchor, c11, c11), tr(c11, c11, c11)))),
      doc(table(tbody(tr(td(p('<a>foo')), cEmpty), tr(c(2, 1), '<b>')))),
      doc(table(tbody(tr(td(p('foo')), cEmpty, c11), tr(c(2, 1), c11)))),
    ));

  it('makes sure the table is big enough', () => {
    return test(
      doc(table(tbody(tr(cAnchor)))),
      doc(table(tbody(tr(td(p('<a>foo')), cEmpty), tr(c(2, 1), '<b>')))),
      doc(table(tbody(tr(td(p('foo')), cEmpty), tr(c(2, 1))))),
    );
  });
  ...
  it('preserves headers while growing a table', () =>
    test(
      doc(
        table(
          tbody(tr(h11, h11, h11), tr(h11, c11, c11), tr(h11, c11, cAnchor)),
        ),
      ),
      doc(table(tbody(tr(td(p('<a>foo')), cEmpty), tr(c11, c11, '<b>')))),
      doc(
        table(
          tbody(
  ...
  it('will split interfering rowspan cells', () =>
    test(
      ...
      doc(table(tbody(tr('<a>', cEmpty, cEmpty, cEmpty, '<b>')))),
      doc(
        table(
          tbody(
            tr(c11, c11, c11),
  ...
  it('will split interfering colspan cells', () =>
    test(
      doc(
        table(tbody(tr(c11, cAnchor, c11), tr(c(2, 1), c11), tr(c11, c(2, 1)))),
      ),
